...
public class SidebarPanel extends JPanel {
    ...
    public SidebarPanel() {
        ...
        setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, UIStyle.BORDER_COLOR));
        ...
        // Menu items panel
        menuPanel = new JPanel(new MigLayout("fillx, wrap, insets 12 16 12 16", "[fill]", "[]6[]"));
    ...
    private JPanel createHeader() {
        JPanel header = new JPanel(new MigLayout("fillx, insets 24 20", "[grow]", "[]"));
        header.setOpaque(false);

        JLabel logoLabel = new JLabel("🎓");
        logoLabel.setFont(new Font(UIStyle.FONT_FAMILY, Font.PLAIN, 32));
        header.add(logoLabel, "growx 0, gapright 12");

        titleLabel = new JLabel("ERP System");
        titleLabel.setFont(UIStyle.FONT_H3);
    ...
    private JButton createMenuButton(SidebarItem item) {
        ...
        btn.setText("<html><span style='font-size:18px;'>" + item.icon +
                "</span> <span style='margin-left:14px;'>" + item.label + "</span></html>");
    ...
    private void toggleCollapse() {
        ...
        for (SidebarItem item : menuItems) {
            ...
            if (isCollapsed) {
                ...
                btn.setToolTipText(item.label); // Add tooltip for collapsed mode
            } else {
                btn.setText("<html><span style='font-size:18px;'>" + item.icon +
